...
function showPhoto(index){photoIndex=(index+gallery.length)%gallery.length;const original=gallery[photoIndex].querySelector('img');photo.classList.remove('is-changing');photo.onload=()=>photo.classList.remove('is-changing');photo.src=gallery[photoIndex].href;photo.alt=original.alt;document.querySelector('#photo-caption').textContent=original.alt;document.querySelector('#photo-counter').textContent=`${photoIndex+1} / ${gallery.length}`;requestAnimationFrame(()=>photo.classList.add('is-changing'));}
...
dialog.addEventListener('keydown',e=>{if(e.key==='ArrowRight'){e.preventDefault();showPhoto(photoIndex+1);}if(e.key==='ArrowLeft'){e.preventDefault();showPhoto(photoIndex-1);}if(e.key==='Tab'){const focusable=[...dialog.querySelectorAll('button,[href],input,select,textarea,[tabindex]:not([tabindex="-1"])')];const first=focusable[0],last=focusable[focusable.length-1];if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}}});dialog.addEventListener('click',e=>{if(e.target===dialog){const r=dialog.getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom)dialog.close();}});dialog.addEventListener('close',()=>{document.body.classList.remove('modal-open');photo.classList.remove('is-changing');opener?.focus();});
